...
(function () {
    ...
    angular
        .module("ineelMT")
        .controller("ConsultaBecariosInternosGetCtrl", [
            "AuthService",
            "$scope",
            "$state",
            "$timeout",
            "MenuService",
            "buscarInformesBecariosService", "globalGet", '$uibModal', '$filter',
            "DTOptionsBuilder", "DTColumnBuilder", "$compile", "authInterceptorService", "$rootScope", ConsultaBecariosInternosGetCtrl]);

    function ConsultaBecariosInternosGetCtrl(AuthService, $scope, $state, $timeout, MenuService, buscarInformesBecariosService, globalGet, $uibModal, $filter,
        ...
        $scope.loadTablaBecariosInternos = function () {
            ...
            tablaBecariosInternos.dtOptions = DTOptionsBuilder.newOptions().withOption('ajax', {
                ...
                data: {
                    "nameColumns": "nombrePersona;fechaInicioBeca;institucion.descripcion;becaInterna.descripcion",
        ...
        function columnaBecario(data, type, full, meta) {
            return '<td>' +
                '<a ng-click="setVariable(' + '&#39;' + 'consultaBecariosInternos' + '&#39;' + ', busqueda)" ui-sref="ConsultaBecarioInternoDetails({id: ' + data.becarioInternoId + '})" title="Detalles">'
                + data.nombrePersona +
                '</a>'
            '</td>'
        }
        ...
        buscarInformesBecariosService.getTiposBecasInternas().then(
            function (result) {
        ...
        $scope.busqueda = MenuService.getVariable('consultaBecariosInternos');
        if ($scope.busqueda == null) {
            ...
            $scope.paramsDT = JSON.parse(localStorage.getItem('tablaConsultaBecariosInternos' + window.location.pathname)); //Recuperamos los parametros de filtrado en el DT
            if ($scope.paramsDT == null) {
                $scope.paramsDT = {};  //Puede que sean nulos, en dado caso se mostrara la pagina 1
                $scope.paramsDT.displayStart = 0;
            }
            $timeout($scope.buscar(), 0);
            MenuService.deleteVariable('consultaBecariosInternos');
        }
        ...
        function stateLoadCallback(settings) {
            ...
            if ($scope.paramsDT != null) {
                ...
                return JSON.parse(localStorage.getItem('tablaConsultaBecariosInternos' + window.location.pathname))
            }
        }
