(function () {
    ...
    angular
        ...
        .controller('HistorialOportunidadDetailsCtrl', [
            ...
            HistorialOportunidadDetailsCtrl
        ]);

    function HistorialOportunidadDetailsCtrl($scope, $stateParams, AuthServiceCR, MenuService, $uibModal, $state, OportunidadNegocioCRService) {

        $scope.authentication = AuthServiceCR.authentication;
        $scope.oportunidad = null;

        $scope.idRol = MenuService.getRolId();
        $scope.nombreEmpleado = AuthServiceCR.authentication.nombreCompleto;


        var Id = $stateParams.id;

        $scope.getOportunidadHistorico = function () {
            OportunidadNegocioCRService.getOportunidadHistorico2(Id).then(
                function (result) {
                    $scope.oportunidadesHistoricos = result.data;
                },
                function (err) {
                    toastr.error(err);
                });
        ...
        $scope.obteneron = function () {
            OportunidadNegocioCRService.getOportunidad(Id).then(
                function (result) {
        ...
        $scope.cambiarestado = function () {
            ...
            OportunidadNegocioCRService.updateOportunidad($scope.oportunidad).then(
                function (result) {
                    ...
                    $scope.obteneron();
                    $scope.formcambioestado.$setPristine();
                },
                function (err) {
                    toastr.error(err);
                }
        ...
        searchType.string = function (data) {
            return !data ?
                '' :
                typeof data === 'string' ?
                    removeAccents(data) : //la funcion es global, esta en globalINEEL.js
                    data;
        };
        ...
        searchType.html = function (data) {
            return !data ?
                '' :
                typeof data === 'string' ?
                    removeAccents(data.replace(/<.*?>/g, '')) :  //la funcion es global, esta en globalINEEL.js
                    data;
        };
