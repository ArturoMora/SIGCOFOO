(function () {
    ...
    angular
        .module("ineelCH")
        .controller("becariodirigidoCtrlGet", ["AuthService", "DTOptionsBuilder", "$scope","$rootScope", "BecarioDirigidoService", "$uibModal","$state", becariodirigidoCtrlGet]);

    function becariodirigidoCtrlGet(AuthService, DTOptionsBuilder, $scope, $rootScope, BecarioDirigidoService, $uibModal, $state) {
        ...
        $rootScope.setVariable("CHficha", "fichapersonal.becariodirigido");
        //Obtener los servicios de autenticacion
        $scope.authentication = AuthService.authentication;
        $scope.editarGestion = 0;

        $scope.paramsDT = JSON.parse(localStorage.getItem('CHbecarioDirig' + window.location.pathname)); //Recuperamos los parametros de filtrado en el DT
        ...
        function stateSaveCallback(settings, data) {
            var stado = $('#CHbecarioDirig').DataTable().state();
            localStorage.setItem('CHbecarioDirig' + window.location.pathname, JSON.stringify(stado))
        }
        ...
        BecarioDirigidoService.GetBecariosDeInvestigadores($scope.numEmp).then(
            function (result) {
        ...
        $scope.open = function (registro) {
            $scope.descripcionRow = "Becario Dirigido";
        ...
        $scope.delete = function (registro, $uibModalInstance) {
            BecarioDirigidoService.delete(registro.becarioDirigidoId).then(
                    function (result) {
