(function () {
    ...
    angular
        .module("ineelCH")
        .controller("sniGetCtrl", ["AuthService", "$scope", "$rootScope", "DTOptionsBuilder", "SNIService","$uibModal",'$state', sniGetCtrl]);

    function sniGetCtrl(AuthService, $scope, $rootScope, DTOptionsBuilder, SNIService, $uibModal, $state) {
        ...
        $rootScope.setVariable("CHficha", "fichapersonal.sni");

        //paginado, filtros y demas cosas de un estado del Datatable
        $scope.paramsDT = JSON.parse(localStorage.getItem('CHsniGet' + window.location.pathname)); //Recuperamos los parametros de filtrado en el DT
        ...
        function stateSaveCallback(settings, data) {
            var stado = $('#CHsniGet').DataTable().state();
            localStorage.setItem('CHsniGet' + window.location.pathname, JSON.stringify(stado))
        }
        ...
        function stateLoadCallback(settings) {
            if ($scope.paramsDT != null) {
                ...
                return JSON.parse(localStorage.getItem('CHsniGet' + window.location.pathname))
            }
        ...
        SNIService.getbyclave($scope.numEmp).then(
            function (result) {
        ...
        $scope.delete = function (sni,$uibModalInstance) {
                SNIService.deleteSNI(sni.sniId).then(
                        function (result) {
                            var idx = ($scope.registrosni.indexOf(sni));
                            $scope.registrosni.splice(idx, 1);
        ...
        $scope.open = function(sni){
             $scope.descripcionRow = sni.areaSNI.descripcion;
            var modalInstance = $uibModal.open({
                templateUrl: 'app/vistasGenericas/eliminacionFisica.html',
                   controller: function ( $uibModalInstance) {
                      
                       $scope.ok = function () {
                        $scope.delete(sni,$uibModalInstance);
                       };
