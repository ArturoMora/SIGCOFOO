(function () {
    ...
    angular
        .module("ineelCH")
        .controller("experienciadocenteCtrlGet", ["AuthService", "$scope", "$rootScope", "ExperienciaDocenteService", "DTOptionsBuilder", "$uibModal","$state", experienciadocenteCtrlGet]);

    function experienciadocenteCtrlGet(AuthService, $scope, $rootScope, ExperienciaDocenteService, DTOptionsBuilder, $uibModal, $state) {
        ...
        $rootScope.setVariable("CHficha", "fichapersonal.experienciadocente");
        //Obtener los servicios de autenticacion
        $scope.authentication = AuthService.authentication;
        $scope.editarGestion = 0;

        //paginado, filtros y demas cosas de un estado del Datatable
        $scope.paramsDT = JSON.parse(localStorage.getItem('CHExperienciaDocenteGet' + window.location.pathname)); //Recuperamos los parametros de filtrado en el DT
        ...
        function stateSaveCallback(settings, data) {
            var stado = $('#CHExperienciaDocenteGet').DataTable().state();
            localStorage.setItem('CHExperienciaDocenteGet' + window.location.pathname, JSON.stringify(stado))
        }
        ...
        function stateLoadCallback(settings) {
            if ($scope.paramsDT != null) {
                ...
                return JSON.parse(localStorage.getItem('CHExperienciaDocenteGet' + window.location.pathname))
            }
        ...
        ExperienciaDocenteService.getbyclave($scope.numEmp).then(
            function (result) {
        ...
        $scope.delete = function (registro, $uibModalInstance) {
            ExperienciaDocenteService.delete(registro.experienciaDocenteId).then(
                    function (result) {
        ...
        $scope.navegador = function () {
            $state.go("experienciadocenteEdit");
        };
    }
})();