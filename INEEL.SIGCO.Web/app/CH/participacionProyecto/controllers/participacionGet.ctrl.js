(function () {
    ...
    angular
        .module("ineelCH")
        .controller("participacionCtrlGet", ["AuthService", "$scope", "$rootScope", "ParticipacionService","DTOptionsBuilder", "$uibModal",'$state', participacionCtrlGet]);

    function participacionCtrlGet(AuthService, $scope, $rootScope, ParticipacionService, DTOptionsBuilder, $uibModal, $state) {
        ...
        $rootScope.setVariable("CHficha", "fichapersonal.participacion");

        //paginado, filtros y demas cosas de un estado del Datatable
        $scope.paramsDT = JSON.parse(localStorage.getItem('CHParticipacionGet' + window.location.pathname)); //Recuperamos los parametros de filtrado en el DT
        ...
        function stateSaveCallback(settings, data) {
            var stado = $('#CHParticipacionGet').DataTable().state();
            localStorage.setItem('CHParticipacionGet' + window.location.pathname, JSON.stringify(stado))
        }
        ...
        function stateLoadCallback(settings) {
            if ($scope.paramsDT != null) {
                ...
                return JSON.parse(localStorage.getItem('CHParticipacionGet' + window.location.pathname))
            }
        ...
        ParticipacionService.getbyclave($scope.numEmp).then(
            function (result) {
                $scope.registros = result.data;
                $scope.loading = false;
            },
            function (err) {
                toastr.error("No se han podido cargar los registros de participación en proyecto.");
            }
            );

        $scope.open = function (registro) {
            $scope.descripcionRow = registro.proyecto.nombre;
        ...
        $scope.delete = function (registro, $uibModalInstance) {
            ParticipacionService.delete(registro.personalProyectoId).then(
                    function (result) {
