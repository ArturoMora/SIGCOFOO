(function () {
    ...
    angular
        .module("ineelCH")
        .controller("tesisdirigidaGetCtrl", ["AuthService", "$scope", "$rootScope", "TesisDirigidaService", "DTOptionsBuilder","$uibModal",'$state', tesisdirigidaGetCtrl]);

    function tesisdirigidaGetCtrl(AuthService, $scope, $rootScope, TesisDirigidaService, DTOptionsBuilder, $uibModal, $state) {
        ...
        $rootScope.setVariable("CHficha", "fichapersonal.tesisdirigida");

        //paginado, filtros y demas cosas de un estado del Datatable
        $scope.paramsDT = JSON.parse(localStorage.getItem('CHTesisDirigidaGet' + window.location.pathname)); //Recuperamos los parametros de filtrado en el DT
        ...
        function stateSaveCallback(settings, data) {
            var stado = $('#CHTesisDirigidaGet').DataTable().state();
            localStorage.setItem('CHTesisDirigidaGet' + window.location.pathname, JSON.stringify(stado))
        }
        ...
        function stateLoadCallback(settings) {
            if ($scope.paramsDT != null) {
                ...
                return JSON.parse(localStorage.getItem('CHTesisDirigidaGet' + window.location.pathname))
            }
        ...
        TesisDirigidaService.getbyclave($scope.numEmp).then(
            function (result) {
                $scope.registros = result.data;
                $scope.loading = false;
            },
            function (err) {
                toastr.error("No se han podido cargar los registros de tesis dirigida.");
            }
            );

        $scope.open = function (registro) {
            $scope.descripcionRow = "Tesis Dirigida";
        ...
        $scope.delete = function (registro, $uibModalInstance) {
            TesisDirigidaService.delete(registro.tesisDirigidaId).then(
                    function (result) {
