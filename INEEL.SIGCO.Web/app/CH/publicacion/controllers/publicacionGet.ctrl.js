(function () {
    ...
    angular
        ...
        .controller("publicacionCtrlGet", ["AuthService", "$scope", "DTOptionsBuilder", "$rootScope", "PublicacionService", "$uibModal", "$state", "DTColumnDefBuilder", publicacionCtrlGet]);

    function publicacionCtrlGet(AuthService, $scope, DTOptionsBuilder, $rootScope, PublicacionService, $uibModal, $state, DTColumnDefBuilder) {
        ...
        $rootScope.setVariable("CHficha", "fichapersonal.publicacion");
        ...
        PublicacionService.getbyclave($scope.numEmp).then(
            ...
            function (err) {
                toastr.error("No se han podido cargar los registros de art\u00edculos.");
            }
            );
