(function () {
    ...
    angular
        .module("ineelGI")
        .controller("ideaInnovadoraDetails", ['AuthService', '$scope', '$stateParams', 'IdeainnovadoraService', "$rootScope", "MenuService", "comiteService", "miembrosGEService", "solicitudesGIService", "bitacoraSolicitudService", "$state", "correoNotificacionService", "EvaluadorIdeaService", "tipoAccesoService", IdeaInnovadoraDetails]);

    function IdeaInnovadoraDetails(AuthService, $scope, $stateParams, IdeainnovadoraService, $rootScope, MenuService, comiteService, miembrosGEService, solicitudesGIService, bitacoraSolicitudService, $state, correoNotificacionService, EvaluadorIdeaService, tipoAccesoService) {
        ...
        $scope.notificar = function () {
            ...
            bitacoraSolicitudService.AddBitacoraSolicitud(Bitacora).then(
                function(res){
                    solicitudesGIService.notificar(noti);
        ...
        $scope.Acciones = function (accion) {
            ...
            if ($scope.rolId == 1029) {
                if ($scope.registro.comentariosEval == null || $scope.registro.comentariosEval == undefined || $scope.registro.comentariosEval=="") {
                    ...
                    EvaluadorIdeaService.UpdateComentarios(evaluador).then(
                        function (result) {
                            ...
                            var Bitacora = {
                                ...
                                "FechaMovimiento": new Date('dd/MM/yyyy'),
                                "ClavePersona": $scope.ClavePersonaLogin,
                                "Descripcion": "Pre-aprobada por el evaluador",
                                "EstadoFlujoId": 14,
                                "Justificacion": $scope.justificacion
                            }
                            bitacoraSolicitudService.AddBitacoraSolicitud(Bitacora);
