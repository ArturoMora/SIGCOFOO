(function () {
    ...
    angular
        ...
        .controller("facturacionRealPorUnidadCtrl", [
            ...
            'comunService',
            '$filter',
            facturacionRealPorUnidadCtrl
        ]);

    function facturacionRealPorUnidadCtrl(AuthService, $scope, $state,
        $stateParams, 
        comunService, $filter) {
        $scope.registros = [];
        $scope.max = 100;
        ...
        $scope.cargarDaros = function (year) {
            comunService.proyectos_FacturacionRealGroupByUnidad(year).then(
                function (result) {
