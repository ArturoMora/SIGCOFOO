...
(function () {
    ...
    angular.module("directivasSIGCO")
        .controller('uoModalConvenioByAliadoCtrl', ['$scope', '$uibModal', uoModalConvenioByAliadoCtrl])
        .controller("ConveniosByAliadoModalFilterGetCtrl",
        ["$scope", "$state", "$stateParams",
            "$uibModal", "$uibModalInstance",
            "$http", "globalGet",
            ConveniosByAliadoModalFilterGetCtrl])
        .directive('conveniosByAliadoSelect', function () {
            return {
                restrict: 'ACE',
                templateUrl: function (elem, attrs) {
                    return attrs.templateUrl || 'app/vistasGenericas/directivas/buscarConveniosByAliadoDirective/buscarConveniosByAliado.html';
                },
                controller: "uoModalConvenioByAliadoCtrl",
                require: 'ngModel',
                scope: {
                    arreglo: "=?enarreglo",
                    tipo: "=?tipo",
    ...
    function ConveniosByAliadoModalFilterGetCtrl($scope, $state, $stateParams, $uibModal, $uibModalInstance,
        $http, globalGet) {
        ...
        service.GetAllnodes = function () {
            ...
            var endPoint = API + "Convenio/GetAllByAliado/" + $scope.aliadoId;
    ...
    ;
    function uoModalConvenioByAliadoCtrl($scope, $uibModal) {
        $scope.elementoSeleccionadoArray = [];
        $scope.openModalConvenioByAliado = function () {
            $scope.arreglo = typeof $scope.arreglo === 'undefined' ? false : $scope.arreglo;
            $scope.tipo = typeof $scope.tipo === 'undefined' ? 0 : $scope.tipo;
            //$scope.aliadoId = typeof $scope.aliadoId === 'undefined' ? 0 : $scope.aliadoId;
            $scope.selectItemConvenioByAliado = {};
            var modalInstance = $uibModal.open({
                size: 'lg',
                templateUrl: 'app/vistasGenericas/directivas/buscarConveniosByAliadoDirective/buscarConveniosByAliadoModalFilterGet.html',
                controller: 'ConveniosByAliadoModalFilterGetCtrl',
                resolve: {
                    selectItemConvenioByAliado: function () {
                        return $scope.selectItemConvenioByAliado;
                    }
                },
                scope: $scope
            });
        ...
        $scope.existe = function (registros, selectedItem) {
            ...
            for (var i = 0; i < registros.length; i++) {
                if (registros[i].ConvenioByAliadoId == selectedItem.ConvenioByAliadoId) {
                    return true;
                }
            }
            return false;
