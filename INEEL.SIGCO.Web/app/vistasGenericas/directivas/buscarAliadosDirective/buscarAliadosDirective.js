...
(function () {
    ...
    angular.module("directivasSIGCO")
        .controller('uoModalAliadoCtrl', ['$scope', '$uibModal', uoModalAliadoCtrl])
        .controller("AliadosModalFilterGetCtrl",
        ["$scope", "$state", "$stateParams",
            "$uibModal", "$uibModalInstance",
            "$http", "globalGet",
            AliadosModalFilterGetCtrl])
        .directive('aliadosSelect', function () {
            return {
                restrict: 'ACE',
                templateUrl: function (elem, attrs) {
                    return attrs.templateUrl || 'app/vistasGenericas/directivas/buscarAliadosDirective/buscarAliados.html';
                },
                controller: "uoModalAliadoCtrl",
                require: 'ngModel',
                scope: {
                    arreglo: "=?enarreglo",
                    ngModel: '=',
    ...
    function AliadosModalFilterGetCtrl($scope, $state, $stateParams, $uibModal, $uibModalInstance,
        $http, globalGet) {
        ...
        service.GetAllnodes = function () {
            var endPoint = API + "Aliado/GetAllForModal";
    ...
    ;
    function uoModalAliadoCtrl($scope, $uibModal) {
        $scope.elementoSeleccionadoArray = [];
        $scope.openModalAliado = function () {
            $scope.arreglo = typeof $scope.arreglo === 'undefined' ? false : $scope.arreglo;
            $scope.selectItemAliado = {};
            var modalInstance = $uibModal.open({
                size: 'lg',
                templateUrl: 'app/vistasGenericas/directivas/buscarAliadosDirective/buscarAliadosModalFilterGet.html',
                controller: 'AliadosModalFilterGetCtrl',
                resolve: {
                    selectItemAliado: function () {
                        return $scope.selectItemAliado;
                    }
                },
                scope: $scope
            });
        ...
        $scope.existe = function (registros, selectedItem) {
            ...
            for (var i = 0; i < registros.length; i++) {
                if (registros[i].aliadoId == selectedItem.aliadoId) {
                    return true;
                }
            }
            return false;
