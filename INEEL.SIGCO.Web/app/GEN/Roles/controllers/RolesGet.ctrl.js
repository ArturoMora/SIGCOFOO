...
(function () {
    ...
    app.controller("RolesGetCtrl", ["$scope", "RolesService", "$uibModal", "DTOptionsBuilder", "DTColumnBuilder", "DTColumnDefBuilder", RolesGetCtrl]);
     
    function RolesGetCtrl($scope, RolesService, $uibModal, DTOptionsBuilder, DTColumnBuilder, DTColumnDefBuilder) {
        $scope.dtInstance = {};
        //Variables de carga
        $scope.loading = true;
            ...
            var modalInstance = $uibModal.open({
                ...
                controller: function ($uibModalInstance) {
                    $scope.ok = function () {
                        registro = {
                            "rolid": id,
                            "descripcion": $scope.descripcionRow,
                            
                            "estado": _estado
                        };
                        RolesService.UpdateEstado(registro);
                    ...
                    $scope.cancel = function () {
                        ...
                        for (var i = 0; i < $scope.registrosRoles.length; i++) {
                            if ($scope.registrosRoles[i].rolId == id) {
                                $scope.registrosRoles[i].estado = estado;
                            }
                        }
                        $uibModalInstance.dismiss('cancel');
                    };
