...
namespace INEEL.WebAPI.Controllers.CP
{
    ...
    {
        private static readonly ILog log = LogManager.GetLogger(typeof(PreguntasController));
        private PreguntasRepository _entityRepo;
        ...
        public async Task<IHttpActionResult> GetByComunidad2(int Id)
        {
            ...
            {
                log.Info(new MDCSet(this.ControllerContext.RouteData), new InfoException(Id));
                var entity = await _entityRepo.GetByComunidad2(Id);
                return Ok(entity);
            }
            catch (Exception e)
            {
        ...
        [Authorize]
        [HttpPost]
        public async Task<IHttpActionResult> Create([FromBody]Preguntas model)
        {
        ...
        [Authorize]
        [HttpPut]
        public async Task<IHttpActionResult> Update([FromBody]Preguntas model)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
