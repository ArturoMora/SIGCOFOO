...
{
    public class MapasRutaController : ApiController
    {
        private MapasRutaRepository _entityRepo;
        private static readonly ILog log = LogManager.GetLogger(typeof(MapasRutaController));
        public MapasRutaController()
        {
            _entityRepo = new MapasRutaRepository();
        ...
        [HttpGet]
        public async Task<IHttpActionResult> CountMapasRuta()
        {
            try
            {
                log.Info(new MDCSet(this.ControllerContext.RouteData));
                var entities = await _entityRepo.CountMapasRuta();
                return Ok(entities);
            }
            catch (Exception e)
            {
        ...
        [HttpPost]
        public async Task<IHttpActionResult> GetAllConsultaPorOC(MapasRuta param)
        {
            try
            {
                log.Info(new MDCSet(this.ControllerContext.RouteData));
        ...
        [Authorize]
        [HttpPost]
        public async Task<IHttpActionResult> Create([FromBody]MapasRuta model)
        {
        ...
        [Authorize]
        [HttpPut]
        public async Task<IHttpActionResult> Update([FromBody]MapasRuta model)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
