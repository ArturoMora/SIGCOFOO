...
{
    public class AgendaCPController : ApiController
    {

        private static readonly ILog log = LogManager.GetLogger(typeof(CategoriaCPController));
        private AgendaCPRepository _entityRepo;

        public AgendaCPController()
        {
            _entityRepo = new AgendaCPRepository();
        }
        ...
        [Authorize]
        [HttpPost]
        public async Task<IHttpActionResult> Create([FromBody]Agenda model)
        {
        ...
        [Authorize]
        [HttpPost]
        public async Task<IHttpActionResult> RegistrayRecupera([FromBody]Agenda model)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            ...
            {
                log.Info(new MDCSet(this.ControllerContext.RouteData));
                var obj =   await _entityRepo.RegistraYRecuperaObjeto(model);
                return Ok(obj);
            }
            catch (Exception e)
            {
        ...
        [Authorize]
        [HttpPut]
        public async Task<IHttpActionResult> Update([FromBody]Agenda model)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
        ...
        [Authorize]
        [HttpPut]
        public async Task<IHttpActionResult> UpdateEstado([FromBody] Agenda model)
        {
            try
            {
                log.Info(new MDCSet(this.ControllerContext.RouteData));
