...
{
    public class TemasInnovacionController : ApiController
    {
        private static readonly ILog log = LogManager.GetLogger(typeof(TemasInnovacionController));
        private TemasInnovacionRepository _entityRepo;
        
        public TemasInnovacionController()
        {
            _entityRepo = new TemasInnovacionRepository();
        }
        ...
        [HttpGet]
        public async Task<IHttpActionResult> CountTemasInnovacion()
        {
            try
            {
                log.Info(new MDCSet(this.ControllerContext.RouteData));
                var entities = await _entityRepo.CountTemasInnovacion();
                return Ok(entities);
            }
            catch (Exception e)
            {
        ...
        [HttpPost]
        public async Task<IHttpActionResult> GetAllConsultaPorOC(TemasInnovacion param)
        {
            try
            {
                log.Info(new MDCSet(this.ControllerContext.RouteData));
        ...
        [Authorize]
        [HttpPost]
        public async Task<IHttpActionResult> Create([FromBody]TemasInnovacion model)
        {
        ...
        [Authorize]
        [HttpPut]
        public async Task<IHttpActionResult> Update([FromBody]TemasInnovacion model)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
