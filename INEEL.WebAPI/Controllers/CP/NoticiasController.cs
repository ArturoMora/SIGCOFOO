...
namespace INEEL.WebAPI.Controllers.CP
{
    ...
    {
        private static readonly ILog log = LogManager.GetLogger(typeof(NoticiasController));
        private NoticiasRepository _entityRepo;
        ...
        [Authorize]
        [HttpPost]
        public async Task<IHttpActionResult> Create([FromBody]Noticia model)
        {
        ...
        [Authorize]
        [HttpPut]
        public async Task<IHttpActionResult> Update([FromBody]Noticia model)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
