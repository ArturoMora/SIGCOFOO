...
namespace INEEL.WebAPI.Controllers.CP
{
    ...
    {
        private static readonly ILog log = LogManager.GetLogger(typeof(TipoDocumentoController));
        private TipoDocumentoRepository _entityRepo;
        
        public TipoDocumentoController()
        {
            _entityRepo = new TipoDocumentoRepository();
        ...
        [HttpGet]
        public async Task<IHttpActionResult> GetByEstado()
        {
            try
            {
                log.Info(new MDCSet(this.ControllerContext.RouteData));
                var entities = await _entityRepo.GetByEstado();
                return Ok(entities);
            }
            catch (Exception e)
            {
        ...
        [Authorize]
        [HttpPost]
        public async Task<IHttpActionResult> Create([FromBody]TipoDocumento model)
        {
        ...
        [Authorize]
        [HttpPut]
        public async Task<IHttpActionResult> Update([FromBody]TipoDocumento model)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
