...
{
    public class ListaOCController : ApiController
    {

        private static readonly ILog log = LogManager.GetLogger(typeof(ListaOCController));
        private ListaOCRepository _entityRepo;
        
        public ListaOCController()
        {
            _entityRepo = new ListaOCRepository();
        ...
        [Authorize]
        [HttpPut]
        public async Task<IHttpActionResult> Update([FromBody]ListaOC model)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
