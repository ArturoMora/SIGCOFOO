...
{
    public class CompetenciasSindController : ApiController
    {
         
        private static readonly ILog log = LogManager.GetLogger(typeof(CompetenciasSindController));

        CompetenciasSindRepository _repository;

        public CompetenciasSindController()
        {
            _repository = new CompetenciasSindRepository();
        }
        ...
        [HttpPost][Authorize] public async Task<IHttpActionResult> Create(CompetenciasSind datos)
        {
            try
            {
        ...
        [Authorize][HttpPut]
        public async Task<IHttpActionResult> Update(CompetenciasSind datos)
        {
            try
            {
                log.Info(new MDCSet(this.ControllerContext.RouteData));
