...
{
    public class ResultadosEvSindController : ApiController
    {

        private static readonly ILog log = LogManager.GetLogger(typeof(ResultadosEvSindController));

        RegistroEvaluacionSindRepository _repository;

        public ResultadosEvSindController()
        {
            _repository = new RegistroEvaluacionSindRepository();
        }
        ...
        [HttpPost][Authorize] public async Task<IHttpActionResult> Create(RegistroEvaluacionesSind datos)
        {
            try
            {
        ...
        [Authorize][HttpPut]
        public async Task<IHttpActionResult> Update(RegistroEvaluacionesSind datos)
        {
            try
            {
                log.Info(new MDCSet(this.ControllerContext.RouteData));
