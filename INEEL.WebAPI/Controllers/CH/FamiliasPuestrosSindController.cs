...
{
    public class FamiliasPuestrosSindController : ApiController
    {
        private static readonly ILog log = LogManager.GetLogger(typeof(FamiliasPuestrosSindController));

        FamiliasSindRepository _repository;

        public FamiliasPuestrosSindController()
        {
            _repository = new FamiliasSindRepository();
        }
        ...
        [HttpPost][Authorize] public async Task<IHttpActionResult> Create(FamiliaPuestosSind datos)
        {
            try
            {
        ...
        [Authorize][HttpPut]
        public async Task<IHttpActionResult> Update(FamiliaPuestosSind datos)
        {
            try
            {
                log.Info(new MDCSet(this.ControllerContext.RouteData));
