...
{
    public class ServicioPorCompetidorController : ApiController { private static readonly ILog log = LogManager.GetLogger(typeof(ServicioPorCompetidorController));

        private ServicioPorCompetidorRepository _entityRepo;
        public ServicioPorCompetidorController()
        {
            _entityRepo = new ServicioPorCompetidorRepository();
       ...
       [HttpGet] public async Task<IHttpActionResult> GetAll(){try { 
        ...
        [HttpPost]
        [Authorize] public async Task<IHttpActionResult> Create([FromBody]ServicioPorCompetidor model)
        {
        ...
        [Authorize][HttpPut]
        public async Task<IHttpActionResult> Update([FromBody]ServicioPorCompetidor model)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
        ...
        [Authorize][HttpPut]
        public async Task<IHttpActionResult> UpdateEstado([FromBody]ServicioPorCompetidor model)
        {
            try { log.Info(new MDCSet(this.ControllerContext.RouteData));
