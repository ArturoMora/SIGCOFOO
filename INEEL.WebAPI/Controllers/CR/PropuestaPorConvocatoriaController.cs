...
{
    public class PropuestaPorConvocatoriaController : ApiController { private static readonly ILog log = LogManager.GetLogger(typeof(PropuestaPorConvocatoriaController));  
        // AYUDA:
        ...
        private PropuestaPorConvocatoriaRepository _entityRepo;
        //private ProductsRepositories _productRepo;
        public PropuestaPorConvocatoriaController()
        {
            _entityRepo = new PropuestaPorConvocatoriaRepository();
       ...
       [HttpGet] public async Task<IHttpActionResult> GetAll(){try { 
        ...
        [AllowAnonymous]
        [HttpGet]
        public async Task<IHttpActionResult> validaConvocatoria(int Id){try{log.Info(new MDCSet(this.ControllerContext.RouteData), new InfoException(Id));
                var entity = await _entityRepo.ValidaConvocatoria(Id);
                return Ok(entity);
            }
            catch (Exception e) { log.Error(new MDCSet(this.ControllerContext.RouteData), e);
                return InternalServerError(e);
        ...
        //[AllowAnonymous]
        //[HttpGet]
        //public async Task<IHttpActionResult> Validador(string propuesta)
        //{
        //    //int ConvocatoriaId = 0;
        //    //String PropuestaId = null;
        //    //if (propuesta != null)
        //    //{
        //    //    //ConvocatoriaId = propuesta.ConvocatoriaId;
        //    //    //PropuestaId = model.propuestaId;
        //    //}
        //    try
        //    {
        //        var campo = await _entityRepo.Validador(propuesta);
        //        return Ok(campo);
        //    }
        //    catch (Exception e)
        //    {
        ...
        [HttpPost]
        [Authorize] public async Task<IHttpActionResult> Create([FromBody]PropuestaPorConvocatoria model)
        {
        ...
        [Authorize][HttpPut]
        public async Task<IHttpActionResult> Update([FromBody]PropuestaPorConvocatoria model)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
        ...
        [Authorize][HttpPut]
        public async Task<IHttpActionResult> UpdateEstado([FromBody]PropuestaPorConvocatoria model)
        {
            try { log.Info(new MDCSet(this.ControllerContext.RouteData));
