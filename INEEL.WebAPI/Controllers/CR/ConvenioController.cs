...
namespace INEEL.WebAPI.Controllers.CR
{
    ...
    {
        private static readonly ILog log = LogManager.GetLogger(typeof(ConvenioController));
        // AYUDA:
        ...
        private ConvenioRepository _entityRepo;
        //private ProductsRepositories _productRepo;
        public ConvenioController()
        {
            _entityRepo = new ConvenioRepository();
            // _productRepo = new ProductsRepositories();
        }
        
        [HttpGet]
        public async Task<IHttpActionResult> GetAllByAliado(int id)
        {
            try
            {
                var entities = await _entityRepo.GetAllByAliado(id);
                return Ok(entities);
            }
            catch (Exception e)
            {
        ...
        [AllowAnonymous]
        [HttpGet]
        public async Task<IHttpActionResult> GetConveniosVigentes()
        {
            try
            {
                log.Info(new MDCSet(this.ControllerContext.RouteData));
                var conveniosvigentes = await _entityRepo.GetConveniosVigentes();
                return Ok(conveniosvigentes);
            }
            catch (Exception e)
            {
                log.Error(new MDCSet(this.ControllerContext.RouteData), e);
        ...
        [HttpGet]
        public async Task<IHttpActionResult> GetConveniosVigentesByEmpresa(int id)
        {
            try
            {
                log.Info(new MDCSet(this.ControllerContext.RouteData));
                var conveniosvigentes = await _entityRepo.GetConveniosVigentesByEmpresa(id);
                return Ok(conveniosvigentes);
            }
            catch (Exception e)
            {
        ...
        [HttpGet]
        public async Task<IHttpActionResult> GetConveniosNoVigentesByEmpresa(int id)
        {
            try
            {
                log.Info(new MDCSet(this.ControllerContext.RouteData));
                var conveniosvigentes = await _entityRepo.GetConveniosNoVigentesByEmpresa(id);
                return Ok(conveniosvigentes);
            }
            catch (Exception e)
            {
        ...
        [HttpPost]
        [Authorize]
        public async Task<IHttpActionResult> Create([FromBody]Convenio model)
        {
        ...
        [Authorize]
        [HttpPut]
        public async Task<IHttpActionResult> Update([FromBody]Convenio model)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
        ...
        [Authorize]
        [HttpPut]
        public async Task<IHttpActionResult> UpdateEstado([FromBody]Convenio model)
        {
            try
            {
                log.Info(new MDCSet(this.ControllerContext.RouteData));
